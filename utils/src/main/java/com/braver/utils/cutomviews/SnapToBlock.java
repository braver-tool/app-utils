 ...
 * snapToBlock.attachToRecyclerView(yourRecyclerView);
 * snapToBlock.setSnapBlockCallback(snapPosition -> {
 * });
 */
...
public class SnapToBlock extends SnapHelper {
        ...
        if (recyclerView != null) {
            ...
            if (layoutManager != null) {
                if (layoutManager.canScrollHorizontally()) {
                    ...
                    mLayoutDirectionHelper = new LayoutDirectionHelper(ViewCompat.getLayoutDirection(mRecyclerView));
    ...
    public int[] calculateDistanceToFinalSnap(@NonNull RecyclerView.LayoutManager layoutManager, @NonNull View targetView) {
        ...
        if (mSnapBlockCallback != null) {
            if (out[0] == 0 && out[1] == 0) {
                Log.d("#####onBlockSnapped", "onBlockSnapped--->" + layoutManager.getPosition(targetView));
    ...
    public int findTargetSnapPosition(RecyclerView.LayoutManager layoutManager, int velocityX, int velocityY) {
        ...
        Log.d("#####", "VisibleItemPosition--->" + currentPos);
        initItemDimensionIfNeeded(layoutManager);
        //  mScroller.fling(0, 0, velocityX, velocityY, -200, 200, -200, 200);
        mScroller.fling(0, 0, velocityX, velocityY, Integer.MIN_VALUE, Integer.MAX_VALUE, Integer.MIN_VALUE, Integer.MAX_VALUE);
        if (velocityX != 0) {
            int snapPos = mLayoutDirectionHelper.getPositionsToMove(lm, mScroller.getFinalX(), mItemDimension);
            Log.d("#####scrollPos", "scrollPos X--->" + snapPos);
            if (snapPos > currentPos) {
                return currentPos + 1;
            } else {
                return snapPos;
            }
        ...
        if (velocityY != 0) {
            Log.d("#####scrollPos", "scrollPos Y--->" + mLayoutDirectionHelper.getPositionsToMove(lm, mScroller.getFinalY(), mItemDimension));
            return mLayoutDirectionHelper.getPositionsToMove(lm, mScroller.getFinalY(), mItemDimension);
        }
    ...
    public View findSnapView(RecyclerView.LayoutManager layoutManager) {
        ...
        View snapView = (snapPos == RecyclerView.NO_POSITION) ? null : layoutManager.findViewByPosition(snapPos);
    ...
    private int calcTargetPosition(LinearLayoutManager layoutManager) {
        ...
        if (firstVisiblePos >= mPriorFirstPosition) {
            ...
            // SnapHelper only snaps to existing views and will not scroll to a non-exist one.
     ...
     * Method used to set dimensions
     * Declared In SnapToBlock.java
     */
    ...
    private class LayoutDirectionHelper {
        ...
        int getScrollToAlignView(View targetView) {
            return (mIsRTL) ? mOrientationHelper.getDecoratedEnd(targetView) - mRecyclerView.getWidth() : mOrientationHelper.getDecoratedStart(targetView);
        }
